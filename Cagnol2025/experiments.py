...
def create_experiments_stc(model):
    return [
        ...
        MeasureSizeTuning(model, ParameterSet({'num_sizes': 12, 'max_size': 5.0, 'log_spacing': True, 'orientations': [0], 'positions': [(0,0)],
                                               'spatial_frequency': 0.8, 'temporal_frequency': 2, 'grating_duration': 2*143*7, 'contrasts': [10, 50], 'num_trials': 10, 'shuffle_stimuli': True})),
    ]
...
def create_experiments_ring_disk(model):
    return [
        ...
        MeasureSizeTuningRing(model, ParameterSet({'num_inner_radius': 10, 'outer_radius': 7.5, 'log_spacing': True, 'orientation': 0,
                                               'spatial_frequency': 0.8, 'temporal_frequency': 2, 'grating_duration': 215*7, 'contrasts': [50], 'num_trials': 10, 'shuffle_stimuli':True})),
        MeasureSizeTuning(model, ParameterSet({'num_sizes': 10, 'max_size': 7.5, 'log_spacing': True, 'orientations': [0], 'positions': [(0,0)],
            'spatial_frequency': 0.8, 'temporal_frequency': 2, 'grating_duration': 215*7, 'contrasts': [50], 'num_trials': 10, 'shuffle_stimuli':True})),
    ]
